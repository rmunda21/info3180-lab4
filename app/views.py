...
def login():
    form = LoginForm()
    if form.validate_on_submit():
        [username, password, token] = form
        user = db.session.execute(db.select(UserProfile).filter_by(username=username.data)).scalar_one()
        if user is not None and check_password_hash(user.password, password.data):
